...
/**
 * Prints out the final statistics for the randomized ROM. 
 * This is because I'm odd and like to gather data about what is occurring and what was generated (mcortez)
 * Pay no mind to me
 * @param rom The rom file to check
 * @param output_stats_dir The directory to write to (CSV format for the files)
 */
void dwr_write_out_stats(dw_rom rom, const char* output_stats_dir)
{
    unsigned long i = 0;
    BOOL newFiles = 0;
    ...
    // If the file is new (newFiles is set to true), then we first write out the CSV header for the data.
    ...
    if (newFiles) {
        fprintf(chestStream, "Hash,Item,Chest Number\n");
    }
    size_t chests_size = sizeof(rom.chests) / sizeof(rom.chests[0]);
    for(i=0; i < chests_size - 1; i++) {
        fprintf(chestStream, "%lu,%s,%lu\n", rom.seed, chest_content_map[rom.chests[i].item - 1], i);
    }

    // Zone layouts
    if (newFiles) {
        fprintf(zoneLayoutsStream, "Hash,Zone,Value\n");
    }
    for (i=0; i < 32; i++) {
        fprintf(zoneLayoutsStream, "%lu,%lu,%d\n", rom.seed, i, rom.zone_layout[i]);
    }

    // Zones
    int zone;
    if (newFiles) {
        fprintf(zonesStream, "Hash,Zone,Enemy\n");
    }
    zone = 0;  /* tantegel zone */
    for (i=0; i < 19; i++) {
        fprintf(zonesStream, "%lu,%d,%d\n", rom.seed, zone, rom.zones[zone * 5 + i]);
    }
    ...
    if (newFiles) {
        fprintf(shopsStream, "Hash,Shop,Item\n");
    }
    for (i=0; i < 7; i++) {
    ...
    if (newFiles) {
        fprintf(locationStream, "Hash,Name,X,Y\n");
    }
    fprintf(locationStream, "%lu,%s,%d,%d\n", rom.seed, "Rainbow Bridge", rom.map.rainbow_bridge->x, rom.map.rainbow_bridge->y);
    fprintf(locationStream, "%lu,%s,%d,%d\n", rom.seed, "Tantegel", rom.map.warps_from[WARP_TANTEGEL].x,rom.map.warps_from[WARP_TANTEGEL].y);
    fprintf(locationStream, "%lu,%s,%d,%d\n", rom.seed, "Charlock", rom.map.warps_from[WARP_CHARLOCK].x,rom.map.warps_from[WARP_CHARLOCK].y);
    fprintf(locationStream, "%lu,%s,%d,%d\n", rom.seed, "Erdrick's Cave", rom.map.warps_from[WARP_ERDRICKS_CAVE].x,rom.map.warps_from[WARP_ERDRICKS_CAVE].y);
    fprintf(locationStream, "%lu,%s,%d,%d\n", rom.seed, "Garinham", rom.map.warps_from[WARP_GARINHAM].x,rom.map.warps_from[WARP_GARINHAM].y);
    fprintf(locationStream, "%lu,%s,%d,%d\n", rom.seed, "Garin's Grave", rom.map.warps_from[WARP_GARINS_GRAVE].x,rom.map.warps_from[WARP_GARINS_GRAVE].y);
    fprintf(locationStream, "%lu,%s,%d,%d\n", rom.seed, "Garin's Grave (Pointless)", rom.map.warps_from[WARP_GARINS_GRAVE_POINTLESS].x,rom.map.warps_from[WARP_GARINS_GRAVE_POINTLESS].y);
    fprintf(locationStream, "%lu,%s,%d,%d\n", rom.seed, "Hauksness", rom.map.warps_from[WARP_HAUKSNESS].x,rom.map.warps_from[WARP_HAUKSNESS].y);
    fprintf(locationStream, "%lu,%s,%d,%d\n", rom.seed, "Jerk Cave", rom.map.warps_from[WARP_JERK_CAVE].x,rom.map.warps_from[WARP_JERK_CAVE].y);
    fprintf(locationStream, "%lu,%s,%d,%d\n", rom.seed, "Kol", rom.map.warps_from[WARP_KOL].x,rom.map.warps_from[WARP_KOL].y);
    fprintf(locationStream, "%lu,%s,%d,%d\n", rom.seed, "Mountain Cave", rom.map.warps_from[WARP_MOUNTAIN_CAVE].x,rom.map.warps_from[WARP_MOUNTAIN_CAVE].y);
    fprintf(locationStream, "%lu,%s,%d,%d\n", rom.seed, "Mountain Cave (Pointless)", rom.map.warps_from[WARP_MOUNTAIN_CAVE_POINTLESS].x,rom.map.warps_from[WARP_MOUNTAIN_CAVE_POINTLESS].y);
    fprintf(locationStream, "%lu,%s,%d,%d\n", rom.seed, "Rimuldar", rom.map.warps_from[WARP_RIMULDAR].x,rom.map.warps_from[WARP_RIMULDAR].y);
    fprintf(locationStream, "%lu,%s,%d,%d\n", rom.seed, "Staff Shrine", rom.map.warps_from[WARP_STAFF_SHRINE].x,rom.map.warps_from[WARP_STAFF_SHRINE].y);
    fprintf(locationStream, "%lu,%s,%d,%d\n", rom.seed, "Swamp North", rom.map.warps_from[WARP_SWAMP_NORTH].x,rom.map.warps_from[WARP_SWAMP_NORTH].y);
    fprintf(locationStream, "%lu,%s,%d,%d\n", rom.seed, "Swamp South", rom.map.warps_from[WARP_SWAMP_SOUTH].x,rom.map.warps_from[WARP_SWAMP_SOUTH].y);
    ...
    if (newFiles) {
        fprintf(enemyStatsStream, "Hash,Enemy Name,Agility,Gold,HP,HR Dodge,Pattern,Resist,Strength,HP\n");
    }
 ...
 * @param output_stats_dir The directory to write out the statistics to
...
uint64_t dwr_randomize(const char* input_file, uint64_t seed, char *flags,
        const char *sprite_name, const char* output_dir, const char* output_stats_dir)
{
    uint64_t crc;
    ...
    if (output_stats_dir != NULL) {
        // Now print out the finalized stats
        dwr_write_out_stats(rom, output_stats_dir);
    }
